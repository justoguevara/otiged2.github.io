...
class Menu {
	constructor() {
		...
		this.vin = [];
		this.selec = null;
		this.ess = {items:{},menu:{},texto:{}};
	}
	add(...tex) { this.tex = this.tex.concat(tex); }
	add_sub_menu(...tex) { this.tex = this.tex.concat(tex); }
	ver() {
		var cad = "";
		for (var i=0; i < this.tex.length; i++) {
			...
			var contenido = ele_texto(this.tex[i],null,this.ess.texto);
...
class Menu_flotante {
	constructor(...bloques) {
		...
		//this.selec = null;
		this.vin = new Array();
		this.cad = "";      // cadena de todos los componentes del bloque.
		this.ess = {};
		this.selec = null;
	...
	add(...bloques) { this.bloques = this.bloques.concat(bloques); }
	add_sub_menu(...bloques) { this.bloques = this.bloques.concat(bloques); }
	ver() {
		var cad = "";
		for (var i=0; i < this.bloques.length; i++) {
			...
			if(i==0){
				var accion = "h.vis." + this.vin[i];   // clic . vinculo . lugar
			}else{
				var accion = "c.v." + this.vin[i];   // clic . vinculo . lugar
			}
			cad += ele_capa(contenido,accion,this.ess.items);
